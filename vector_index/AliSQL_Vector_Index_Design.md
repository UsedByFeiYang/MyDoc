...
> 基于 `sql/vidx/` 与 `include/vidx/` 源码分析，版本：2025，来源：Alibaba AliSQL。
...
AliSQL 向量索引（vidx）是以 **MySQL Daemon Plugin** 形式挂载到 MySQL 8.0 内核的原生向量近邻搜索能力，主要由以下几个模块组成：
...
> 注意：AliSQL 向量索引目前采用**软删除**策略，被删除的节点仍留在图中参与图遍历（用于连通性），仅在返回结果时过滤掉。这避免了图结构重组的高代价，但会随着删除比例增加而降低搜索效率。
